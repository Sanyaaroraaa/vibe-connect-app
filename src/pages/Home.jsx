...
const MissionTimer = memo(({ mins, sessionStarted, startedAt, vibeId }) => {
  ...
  return (
    <span className="fw-bold" style={{ 
      fontSize: '0.85rem', 
      color: (seconds < 60 && (sessionStarted || !isInRoom)) ? '#ff4444' : (isInRoom && !sessionStarted ? '#444' : accent) 
    }}>
      {Math.floor(seconds/60)}m {seconds % 60}s
      {isInRoom && !sessionStarted && <span className="ms-1" style={{fontSize: '0.6rem', opacity: 0.5}}>(PAUSED)</span>}
    </span>
  );
});
...
const SwipeSlider = ({ onComplete, onCancel }) => {
  ...
  const handleMouseMove = (e) => {
    ...
    if (progress > 95) { isDragging.current = false; onComplete(); }
  };
  return (
    <div className="text-center py-2 animate__animated animate__fadeIn">
      <div className="mb-3 d-inline-block p-2 rounded-circle" style={{ backgroundColor: `${accent}11` }}><ShieldCheck size={24} color={accent} /></div>
      <h6 className="fw-black text-uppercase mb-3 small" style={{ letterSpacing: '1px' }}>Slide to Handshake</h6>
      <div id="swipe-track" className="position-relative overflow-hidden mb-3" style={{ height: '54px', background: '#000', border: '1px solid #2f3336', borderRadius: '27px', width: '100%', cursor: 'pointer', touchAction: 'none' }} onMouseMove={handleMouseMove} onTouchMove={handleMouseMove} onMouseUp={() => setSliderValue(0)} onTouchEnd={() => setSliderValue(0)}>
        <div className="position-absolute h-100 start-0" style={{ width: `${sliderValue}%`, background: accent, opacity: 0.3 }} />
        <div className="h-100 d-flex align-items-center justify-content-center text-white-50 small fw-bold">JOIN ROOM {'>>>'}</div>
        <div onMouseDown={() => { isDragging.current = true; }} onTouchStart={() => { isDragging.current = true; }} className="position-absolute d-flex align-items-center justify-content-center shadow-lg" style={{ left: `calc(${sliderValue}% - ${sliderValue > 80 ? '48px' : '2px'})`, width: '48px', height: '48px', background: accent, borderRadius: '50%', top: '2px', color: '#000' }}><ArrowRight size={22} strokeWidth={3} /></div>
      </div>
      <Button onClick={onCancel} variant="link" className="text-white-50 small text-decoration-none p-0 fw-bold">ABORT CONNECTION</Button>
    </div>
  );
};
...
const CreateVibeForm = memo(({ onSignal, hasActiveVibe, onMobileClose, isModal }) => {
  ...
  const timerOptions = [{ label: '5 Mins', value: 5 }, { label: '15 Mins', value: 15 }, { label: '30 Mins', value: 30 }, { label: '1 Hour', value: 60 }];
  ...
  return (
    <div className={`p-4 ${!isModal ? 'mb-4 shadow-lg' : ''}`} style={{ backgroundColor: '#16181c', border: isModal ? 'none' : '1px solid #2f3336', borderLeft: isModal ? 'none' : `6px solid ${hasActiveVibe ? '#333' : accent}`, borderRadius: isModal ? '28px' : '24px' }}>
        <div className="d-flex justify-content-between align-items-center mb-4">
            <h6 className="fw-bold m-0 text-white-50 small text-uppercase d-flex align-items-center gap-2">{hasActiveVibe ? <><Lock size={14}/> Node Busy</> : "Start a Vibe"}</h6>
            {isModal && <div onClick={onMobileClose} className="p-2 rounded-circle bg-black active-click" style={{cursor:'pointer'}}><X size={20} className="text-white" /></div>}
        </div>
        <div className="d-flex flex-wrap gap-2 mb-3">
            {['Walk', 'Study', 'Coffee', 'Gym', 'Other'].map(c => (
                <Button key={c} disabled={hasActiveVibe} onClick={() => setSelectedCat(c)} style={{ backgroundColor: selectedCat === c ? accent : '#000', color: selectedCat === c ? '#000' : '#fff', border: '1px solid #2f3336', borderRadius: 12, fontSize: '0.75rem', padding: '6px 12px' }} className="fw-bold shadow-none active-click">{c}</Button>
            ))}
        </div>
        <Form.Control disabled={hasActiveVibe} as="textarea" rows={1} value={text} onChange={handleTextChange} placeholder="What's the plan?" className="mb-2 shadow-none bg-black border-dark text-white p-3 rounded-4 custom-scrollbar" style={{ resize: 'none', height: '55px', colorScheme: 'dark' }} />
        <div className="d-flex align-items-center gap-2 mb-2 p-2" style={{ backgroundColor: '#000', borderRadius: '12px', border: '1px solid #2f3336' }}>
            <MapPin size={16} color="#888" /><Form.Control disabled={hasActiveVibe} type="text" placeholder="Where?" value={loc} onChange={(e) => setLoc(e.target.value)} className="p-0 border-0 shadow-none text-white bg-transparent custom-placeholder" />
        </div>
        <div className="position-relative mb-4">
            <div className="d-flex align-items-center gap-2 p-2" onClick={() => !hasActiveVibe && setIsTimerOpen(!isTimerOpen)} style={{ backgroundColor: '#000', borderRadius: '12px', border: '1px solid #2f3336', cursor: 'pointer' }}>
                <Clock size={16} style={{ color: '#888' }} /><span style={{ fontSize: '0.9rem', color: '#888' }}>Active for {timerOptions.find(o => o.value === mins)?.label}</span><ChevronDown size={14} className="ms-auto" />
            </div>
            {isTimerOpen && (
              <div className="timer-dropdown animate__animated animate__fadeIn" style={{ position: 'absolute', width: '100%', top: '100%', marginTop: '5px', background: '#1c1f24', borderRadius: '16px', border: '1px solid #3a3f44', zIndex: 9999, overflow: 'hidden' }}>
                {timerOptions.map(o => <div key={o.value} className="p-3 text-white dropdown-item-custom active-click" style={{ cursor: 'pointer' }} onClick={() => { setMins(o.value); setIsTimerOpen(false); }}>{o.label}</div>)}
              </div>
            )}
        </div>
        <Button onClick={() => { onSignal({ text, loc, mins, type: selectedCat }); if(onMobileClose) onMobileClose(); setText(""); setLoc(""); }} disabled={hasActiveVibe || !text} className="w-100 py-3 fw-bold border-0 shadow-none active-click" style={{ backgroundColor: (hasActiveVibe || !text) ? '#222' : accent, color: '#000', borderRadius: '15px' }}>Signal Vibe</Button>
    </div>
  );
});
...
const Home = () => {
  ...
  const [activeConnection, setActiveConnection] = useState(() => localStorage.getItem('activeVibeId') || null);
  const [isMeetupActive, setIsMeetupActive] = useState(() => localStorage.getItem('isMeetupActive') === 'true');
  ...
  useEffect(() => {
    ...
    onSnapshot(doc(db, "users", auth.currentUser.uid), (docSnap) => {
      if (docSnap.exists()) { 
        setMyBlockedUsers(docSnap.data().blockedUsers || []); 
        setMyTrustPoints(docSnap.data().trustPoints || 0); 
      }
    });
  }, []);
  ...
  const handleCreateVibe = async (vibeData) => {
    ...
    try { await createVibe(vibeData, userLoc); toast.success("Vibe Signal Sent!"); } catch (err) { toast.error("Failed to signal"); }
  };

  const handleJoinVibeConfirm = async (item) => {
    if (!userLoc) { toast.info("Waiting for GPS..."); return; }
    try {
        const success = await joinVibe(item.id, userLoc); 
        if (success) { setTimeout(() => { setActiveConnection(item.id); setIsMeetupActive(true); }, 100); }
    } catch (err) { toast.error(err.message || "Join failed"); }
  };
...
const feedVisibleToMe = useMemo(() => {
  ...
  useEffect(() => {
    if (!isMeetupActive) {
      ...
      if (targetBtn && filterIndicatorRef.current) {
        gsap.to(filterIndicatorRef.current, { x: targetBtn.offsetLeft, width: targetBtn.offsetWidth, duration: 0.4, ease: "expo.out" });
      }
    }
  ...
  return (
    <div style={{ backgroundColor: '#000', minHeight: '100vh', width: '100%', color: '#fff', fontFamily: "'Outfit', sans-serif" }}>
      ...
      {/* NAVBAR */}
      <div className="p-4 d-flex justify-content-between align-items-center sticky-top" style={{ backgroundColor: '#000', borderBottom: '1px solid #2f3336', zIndex: 1200 }}>
        <div className="d-flex align-items-center gap-2"><Zap size={28} color={accent} strokeWidth={3} /><h3 className="fw-bold m-0 d-none d-sm-block">connect.</h3></div>
        <div className="d-flex align-items-center gap-2 gap-md-4">
             ...
             <SettingsIcon size={22} className="active-click text-white-50" style={{ cursor: 'pointer' }} onClick={() => setIsSettingsOpen(true)} />
        </div>
      </div>
        ...
        <div className="d-flex justify-content-between small text-white-50">
          ...
          <Col lg={isMeetupActive ? 12 : 8} xs={12}>
            {isMeetupActive ? (
                ...
                {/* STICKY FILTER BAR */}
                <div className="sticky-top py-3" style={{ top: '85px', backgroundColor: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(10px)', zIndex: 1050 }}>
                    <div className="d-flex gap-1 overflow-auto no-scrollbar position-relative mx-auto" style={{ backgroundColor: '#111', borderRadius: '50px', border: '1px solid #2f3336', padding: '4px 10px', width: 'fit-content', maxWidth: '100%' }}>
                        <div ref={filterIndicatorRef} className="filter-indicator" />
                        {baseCategories.map((cat, i) => (<button key={cat.name} ref={el => buttonsRef.current[i] = el} onClick={() => setFilter(cat.name)} className={`filter-btn ${filter === cat.name ? 'active' : ''}`}>{cat.name}</button>))}
                    </div>
                </div>
